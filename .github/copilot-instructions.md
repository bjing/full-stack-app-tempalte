...
**Build & Dev Commands**
- **Backend (Haskell, Stack):** Build and run tests with:
  - `cd backend && stack build`
  - `cd backend && stack test`
  - Run server locally: `cd backend && stack exec tva-exe` (or `stack build && stack exec tva-exe`).
- **Note:** Configuration comes from `backend/stack.yaml` and `backend/package.yaml` (hpack). Do NOT edit the generated `backend/tva.cabal` directly.
- **Frontend (Next.js):**
...
- **Hpack-first Haskell:** `backend/package.yaml` is the source of truth; `tva.cabal` is generated by `hpack`. Update `package.yaml` and `stack.yaml` instead of editing the `.cabal` file.
